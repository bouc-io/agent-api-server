...
CI runs `lint` and `unit-test` from the shared `bouc-io/ci-templates` project. Two
things about that pipeline are worth knowing before you change code here:

- **Lint uses a warning ratchet.** ESLint *errors* always fail the build. Warnings are
  capped by `max_warnings` in `.gitlab-ci.yml`, currently **13** (7
  `no-unused-vars`, 6 `no-explicit-any`). When you clean some up, lower the number.
  Never raise it, and never silence a rule repo-wide to get green.
- **Formatting is enforced.** `format:check` gates, using prettier defaults with no
  config file. Two things to know if you ever reformat: prettier needs *two* passes to
  converge here (it breaks `res.status(x).json({...})` chains on the first pass and
  collapses them on the second), and it can wrap a statement out from under an
  `eslint-disable-next-line`. That is why the `no-require-imports` suppressions in
  `src/lib/tracing.ts`, `src/lib/swagger.ts`, and `src/routes/assignmentRoutes.ts` use
  block `/* eslint-disable */ ... /* eslint-enable */` pairs, which survive re-wrapping.
  Keep them that way.

Integration tests are deliberately not in CI: they need live Postgres, Redis, and
Ollama. Unit tests must stay free of those dependencies.
