...
class ToolRegistry {
     ...
     */
    async getToolDefinitionsFromDB(clientType?: string): Promise<LLMToolDefinition[]> {
        ...
        const dbDefs = tools
            .filter((tool) => this.handlers.has(tool.name) || (tool.requires_client_execution && isCli))
            .map((tool) => ({
