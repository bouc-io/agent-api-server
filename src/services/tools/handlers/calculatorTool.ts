  ...
  async execute(
    ...
    if (!expression || typeof expression !== "string") {
      return {
        ...
        output: null,
        error:
          "Missing or invalid required parameter: expression (must be a string)",
      };
    }
    ...
    try {
      ...
      if (result instanceof Unit) {
        ...
        log.debug(
          { expression: cleanExpression, unitStr },
          "calculator: unit result",
        );
        return {
          success: true,
          output: {
      ...
      if (!isFinite(numericResult)) {
        return {
          ...
          output: null,
          error:
            "Result is not a finite number (infinity or NaN) — check your expression",
        };
      }
      ...
      log.debug(
        { expression: cleanExpression, numericResult, formatted },
