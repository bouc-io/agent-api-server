 ...
 */
export function validateFetchUrl(urlString: string): { valid: true } | { valid: false; error: string } {
    let parsed: URL;
    try {
    ...
    if (host.endsWith('.local')) {
        return { valid: false, error: 'Requests to .local domains are not allowed.' };
    }
