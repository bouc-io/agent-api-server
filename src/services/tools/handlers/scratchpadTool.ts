...
export const scratchpadWriteTool: ToolHandler = {
    ...
    execute: async (args: Record<string, unknown>, context: ToolContext): Promise<ToolResult> => {
        ...
        if (!key) return { success: false, output: null, error: 'A non-empty "key" is required.' };
        if (content.length > MAX_CONTENT) {
            return { success: false, output: null, error: `Content exceeds the ${MAX_CONTENT}-character limit.` };
        }
        try {
            await prisma.scratchpad.upsert({
                where: { assignment_id_key: { assignment_id: context.assignmentId, key } },
                update: { content },
                create: { assignment_id: context.assignmentId, key, content },
            });
            return { success: true, trust: 'trusted', output: { key, bytes: content.length, status: 'saved' } };
        } catch (err) {
            const message = err instanceof Error ? err.message : 'unknown error';
            log.warn({ err: message }, 'scratchpad_write failed');
            return { success: false, output: null, error: `Failed to write scratchpad: ${message}` };
        }
    },
};
    ...
    execute: async (args: Record<string, unknown>, context: ToolContext): Promise<ToolResult> => {
        ...
        try {
            if (key) {
                ...
                if (!row) return { success: true, trust: 'trusted', output: { key, found: false, content: null } };
                return {
                    success: true,
                    trust: 'trusted',
                    output: { key, found: true, content: row.content, updated_at: row.updated_at },
                };
            }
            ...
            return { success: false, output: null, error: `Failed to read scratchpad: ${message}` };
        }
    },
};
