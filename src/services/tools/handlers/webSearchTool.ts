  ...
  for (let attempt = 0; attempt < FALLBACK_MAX_RETRIES; attempt++) {
    try {
      ...
      log.debug(
        {
          ...
          htmlPreview:
            html.slice(0, 500) + (html.length > 500 ? "... [truncated]" : ""),
        },
      ...
      log.debug(
        ...
        "DDG HTML fallback: parsed results",
      );

      if (results.length === 0) {
        throw new Error("DDG HTML fallback returned no parseable results");
      }
      ...
      if (attempt < FALLBACK_MAX_RETRIES - 1) {
        const delay = FALLBACK_BASE_DELAY_MS * Math.pow(2, attempt);
        log.warn(
          {
            attempt: attempt + 1,
...
async function searchViaDDGLite(
  ...
  log.debug({ htmlLength: html.length }, "DDG Lite: raw response received");
...
export const webSearchTool: ToolHandler = {
  ...
  async execute(
    ...
    log.debug(
      { query, maxResults, fetchTopResult, rawArgs: args },
      "web_search: execute called",
    );
    ...
    try {
      ...
      log.info(
        ...
        "DDG scrape search succeeded",
      );
    } catch (primaryError) {
      const primaryMsg =
        primaryError instanceof Error ? primaryError.message : "Unknown error";
      log.warn(
        { query, error: primaryMsg },
        "DDG scrape failed, trying HTML fallback",
      );
    }
      ...
      try {
        ...
        const fallbackMsg =
          fallbackError instanceof Error
            ...
            : "Unknown error";
        log.warn(
          { query, error: fallbackMsg },
          "DDG HTML fallback failed, trying DDG Lite",
        );
      }
    }
    ...
    if (!results) {
      try {
        ...
        log.error(
          { query, error: liteMsg },
          "All DDG search strategies failed",
        );
        return {
          success: false,
          output: null,
    ...
    if (fetchTopResult && results.length > 0) {
      ...
      if (topResultContent) {
        log.debug(
          { url: results[0].url },
          "web_search: fetched top result content",
        );
      }
    }
