  ...
  async execute(
    ...
    if (!memoryClient.isEnabled()) {
      return {
        ...
        output: {
          available: false,
          message:
            "Memory service is not enabled (MEMORY_SERVICE_ENABLED is not set).",
          memories: [],
          count: 0,
        },
      };
    ...
    if (!userId) {
      return {
        ...
        output: null,
        error:
          "Memory search requires a user context (userId not available in this run)",
      };
    }
    ...
    try {
      ...
      const msg =
        (err as Error).message || "Unknown error contacting memory service";
