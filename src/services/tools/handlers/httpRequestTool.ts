...
export const httpRequestTool: ToolHandler = {
  ...
  async execute(
    ...
    const maxResponseChars =
      (args.max_response_chars as number) ?? DEFAULT_MAX_RESPONSE_CHARS;
    ...
    if (!apiId && !customUrl) {
      return {
        ...
        output: null,
        error:
          "Must provide either api_id (for pre-configured APIs) or url (for custom requests)",
      };
    }
    ...
    if (apiId) {
      ...
      log.debug(
        { apiId, apiName: api.name, finalUrl },
        "Using pre-configured API",
      );
    } else {
      // Use custom URL
      finalUrl = customUrl!;
    ...
    try {
        ...
        if (typeof responseData === "string") {
          try {
            ...
            return {
              ...
              output: null,
              error:
                "json_path requires a JSON response body, but the response is not valid JSON",
            };
          }
        }
      ...
      if (serialized.length > maxResponseChars) {
        ...
        responseData =
          serialized.slice(0, maxResponseChars) + "... [truncated]";
        truncated = true;
      }
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (
          ...
          log.warn(
            { url: finalUrl, timeout, duration_ms: duration },
        ...
        log.error(
          { url: finalUrl, error: errorMsg, duration_ms: duration },
          "HTTP request failed",
        );
        return {
          success: false,
      ...
      log.error(
        { url: finalUrl, error: errorMessage, duration_ms: duration },
        "Unexpected error",
      );
      return {
        success: false,
  ...
  try {
    ...
    if (parsed.hostname.endsWith(".local")) {
      return {
        valid: false,
        error:
          "Requests to .local domains are not allowed for security reasons.",
      };
    }
