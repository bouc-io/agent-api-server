...
const DEFAULT_TIMEOUT_MS = parseInt(process.env.SANDBOX_TIMEOUT_MS || '10000', 10);
    ...
    execute: async (args: Record<string, unknown>, _context: ToolContext): Promise<ToolResult> => {
        if (process.env.ALLOW_CODE_EXECUTE !== 'true') {
            return {
                success: false,
                output: null,
                error: 'code_execute is disabled on this server (set ALLOW_CODE_EXECUTE=true to enable).',
            };
        }

        const language = typeof args.language === 'string' ? args.language.toLowerCase() : 'python';
        if (language !== 'python' && language !== 'node') {
            return { success: false, output: null, error: `Unsupported language '${language}'. Supported: python, node.` };
        }
        ...
        try {
            ...
            return { success: false, output: null, error: `Sandbox error: ${message}` };
        }
    },
};
