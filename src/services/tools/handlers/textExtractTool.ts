...
export const textExtractTool: ToolHandler = {
  ...
  async execute(
    ...
    if (
      !["find_all", "find_first", "extract_groups", "replace"].includes(
        operation,
      )
    ) {
      return {
    ...
    if (!validFlags) {
      return {
        success: false,
        output: null,
        error: `Invalid regex flags: "${flags}". Allowed: g, i, m, s, u, d`,
      };
    }
      ...
      case "replace": {
        if (replacement === undefined) {
          return {
            ...
            output: null,
            error:
              "Missing required parameter: replacement (required for replace operation)",
          };
        }
