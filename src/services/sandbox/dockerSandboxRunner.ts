...
import { SandboxRunner, SandboxRunInput, SandboxRunResult } from './sandboxRunner';
...
const MAX_OUTPUT = parseInt(process.env.SANDBOX_MAX_OUTPUT_BYTES || '1000000', 10);
    ...
    async run(input: SandboxRunInput): Promise<SandboxRunResult> {
        ...
        return new Promise<SandboxRunResult>((resolve) => {
            ...
            child.on('error', (err: Error) => {
                ...
                settled = true;
                clearTimeout(timer);
                log.warn({ err: err.message }, 'sandbox spawn failed (is the container runtime available?)');
                resolve({
                    stdout,
