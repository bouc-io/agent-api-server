...
export function buildInstructionPrefix(
  ...
  const memoryInstructions = memories.filter(
    (m) => m.category === "instruction",
  );
  const all = [
...
export function buildKnowledgeSuffix(memories: MemorySearchResult[]): string {
  const distilled = memories.filter(
    (m) => (m.memory_type ?? "distilled") === "distilled",
  );
  const neuralese = memories.filter((m) => m.memory_type === "neuralese");

  const facts = distilled.filter(
    (m) => m.category === "fact" || m.category === "context",
  );
  const profile = distilled.filter(
