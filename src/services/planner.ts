import { LLMClient, LLMMessage, LLMToolDefinition, Plan, TokenAccumulator, createTokenAccumulator, accumulateTokens } from '../types/llm';
 ...
 */
function buildPlannerPrompt(tools?: LLMToolDefinition[], hasMemoryContext?: boolean): string {
...
function validateAndSimplifyPlan(plan: Plan): Plan {
    ...
    if (uniqueTools.size === 1 && toolIntents.length === steps.length) {
        ...
        if (hasDecompositionKeywords) {
            ...
            return {
                goal: plan.goal,
                steps: [{ id: 1, description: plan.goal, tool_intent: toolIntents[0] as string }],
            };
        }
    }
    ...
    if (referencesResult && steps.length > 1) {
        ...
        return {
            goal: plan.goal,
            steps: [{ id: 1, description: plan.goal, tool_intent: lastStep.tool_intent }],
        };
    }
...
export async function executePlanner(
    ...
    const messages: LLMMessage[] = [
        { role: 'system', content: instructionPrefix + plannerPrompt + knowledgeSuffix },
        ...conversationMessages,
        {
            role: 'user',
            content: 'Output the JSON plan now. The "goal" field must restate the LAST user message above in that user\'s own words (their intent) — do NOT echo this instruction into the goal.',
        },
    ];
    ...
    planLog.debug(
        { rawResponse: rawResponse.slice(0, 1000) + (rawResponse.length > 1000 ? '... [truncated]' : '') },
    ...
    if (!rawPlan.goal || rawPlan.goal === 'Unable to parse plan from response') {
        ...
        const retryMessages: LLMMessage[] = [
            { role: 'system', content: 'Output JSON only: {"goal":"...","steps":[{"id":1,"description":"...","tool_intent":"tool_name or null"}]}' },
...
export async function executeReplanner(
    ...
    try {
        ...
        let parsed: { revised?: boolean; reason?: string; goal?: string; steps?: unknown[] };

        try {
            parsed = extractJSON(raw) as typeof parsed;
        } catch {
            replanLog.warn({ raw }, 'Re-planner: failed to parse response — treating as no revision');
            return { revised: false, reason: 'Parse error', tokenUsage };
        }

        if (parsed.revised && parsed.goal && Array.isArray(parsed.steps) && parsed.steps.length > 0) {
            const revisedPlan: Plan = {
                goal: parsed.goal,
                steps: parsed.steps as Plan['steps'],
            };
            replanLog.info({ reason: parsed.reason, newSteps: revisedPlan.steps.length }, 'Re-planner: plan revised');
            return { revised: true, plan: revisedPlan, reason: parsed.reason ?? 'Plan updated', tokenUsage };
        }

        return { revised: false, reason: parsed.reason ?? 'Plan is valid', tokenUsage };
    } catch (err) {
