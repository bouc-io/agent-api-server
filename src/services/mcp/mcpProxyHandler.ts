...
import { ToolHandler, ToolContext, ToolResult, ToolPolicy } from '../../types/tool';
...
export class McpProxyHandler implements ToolHandler {
    ...
    public readonly policy: ToolPolicy = { requires_approval: true, state_changing: true };

    /** Remote MCP output is untrusted external content. */
    public readonly trust: 'untrusted' = 'untrusted';
    ...
    async execute(
        ...
        try {
            ...
            return { success: false, output: null, error: this.withSchemaHint(message) };
        }
    }
}
