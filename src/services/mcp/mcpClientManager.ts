...
const SYNC_INTERVAL_MS = parseInt(process.env.MCP_SYNC_INTERVAL_MS || '60000', 10);
...
export class McpClientManager {
    ...
    private async loadConfigs(): Promise<McpServerConfig[]> {
        ...
        try {
            ...
            const servers = (response.data?.servers ?? []) as Array<Record<string, unknown>>;
            ...
            log.warn({ err: message }, 'Failed to fetch MCP server config from admin-api-server');
            return [];
        }
    }
    ...
    private async connectServer(config: McpServerConfig): Promise<void> {
        const entry: McpServerEntry = { config, status: 'connecting', toolNames: [] };
        ...
        try {
            ...
            for (const tool of tools) {
                ...
                toolRegistry.register(
                    new McpProxyHandler(config.name, tool.name, prefixedName, client, inputSchema)
                );
