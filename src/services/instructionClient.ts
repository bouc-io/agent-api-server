 ...
 */
export async function fetchInstructions(accessToken: string): Promise<ConfigInstruction[]> {
    const adminUrl = process.env.ADMIN_API_URL;
    const portalUrl = process.env.PORTAL_API_URL;

    if (!adminUrl && !portalUrl) {
        log.debug('ADMIN_API_URL and PORTAL_API_URL not set, skipping instruction fetch');
        return [];
    }
    ...
    if (adminResult.status === 'fulfilled') {
        ...
        log.warn({ err: adminResult.reason?.message }, 'Failed to fetch global/org instructions from admin-api-server');
    }

    if (portalResult.status === 'fulfilled') {
        instructions.push(...(portalResult.value.data.instructions ?? []));
    } else {
        log.warn({ err: portalResult.reason?.message }, 'Failed to fetch personal instructions from portal-api-server');
    }
