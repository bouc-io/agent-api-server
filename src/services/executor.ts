 ...
 */
const MAX_EXECUTION_STEPS = parseInt(
  process.env.MAX_EXECUTION_STEPS || "10",
  10,
);
/** Overall wall-clock budget for the executor phase; guards against indefinite loops. */
const EXECUTOR_TIMEOUT_MS = parseInt(
...
async function waitForClientToolResult(
  ...
  return new Promise<ToolResult>((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(
        new Error(
          `Client tool '${toolCallId}' timed out after ${timeoutMs}ms — CLI did not respond`,
        ),
      );
    }, timeoutMs);
    ...
    subscriber.subscribe(channel, (err) => {
      if (err) {
        ...
        reject(
          new Error(
            `Failed to subscribe to client tool channel: ${err.message}`,
          ),
        );
      }
    });
    ...
    subscriber.on("message", (_ch: string, message: string) => {
      ...
      try {
        ...
        resolve({
          success: false,
          output: null,
          error: "Invalid tool result payload from client",
        });
      }
    });
    ...
    (resolve, reject) => {
      ...
      subscriber.subscribe(channel, (err) => {
        if (err) {
          ...
          reject(
            new Error(
              `Failed to subscribe to approval channel: ${err.message}`,
            ),
          );
        }
      });
...
function isRawJsonToolCall(content: string): boolean {
  ...
  try {
    ...
    return (
      typeof parsed.name === "string" && (parsed.parameters || parsed.arguments)
    );
  } catch {
    return false;
  }
...
function extractKeyResult(toolOutput: string): string | null {
  try {
    ...
    if (parsed.output !== undefined && typeof parsed.output !== "object")
      return String(parsed.output);

    // time_date: current_time, format_date, day_of_week (human-friendly formatted string)
    if (typeof parsed.formatted === "string" && parsed.formatted.length > 0)
      return parsed.formatted;

    // time_date: difference
    if (
      typeof parsed.human_readable === "string" &&
      parsed.human_readable.length > 0
    )
      return parsed.human_readable;

    // time_date: convert_timezone → { target: { formatted: "..." } }
    if (parsed.target && typeof parsed.target.formatted === "string")
      return parsed.target.formatted;
    ...
    if (Array.isArray(parsed.results) && parsed.results.length > 0) {
      ...
      if (first && typeof first.snippet === "string")
        return first.snippet.slice(0, 300);
    }

    // http_request → { data: ... } — only handle scalar responses
    if (parsed.data !== undefined) {
      if (typeof parsed.data === "string") return parsed.data.slice(0, 300);
      if (typeof parsed.data === "number" || typeof parsed.data === "boolean")
        return String(parsed.data);
    }

    // fetch_page → { content: "..." }
    if (typeof parsed.content === "string" && parsed.content.length > 0)
      return parsed.content.slice(0, 300);
    ...
    if (Array.isArray(parsed.memories) && parsed.memories.length > 0) {
      ...
      if (first && typeof first.content === "string")
        return first.content.slice(0, 300);
    }
...
function extractConversationContext(messages: LLMMessage[]): string {
  ...
  if (lastUserMsg) {
    parts.push(`Current user request: "${lastUserMsg.content.slice(0, 200)}"`);
  }
  ...
  try {
    ...
    if (needsApproval) {
      ...
      if (autoApprove) {
        ...
        log.info(
          { toolName: toolCall.name, approvalId },
          "Tool: requires approval — parking run",
        );
        ...
        if (!decision.approved) {
          ...
          log.info(
            { toolName: toolCall.name, approvalId, reason: decision.reason },
        ...
        log.info(
          { toolName: toolCall.name, approvalId },
          "Tool: approved — proceeding with execution",
        );
    ...
    if (await toolRegistry.isClientSideTool(toolCall.name)) {
      ...
      result = await waitForClientToolResult(
        runId,
        tc.id,
...
function shouldReplan(
  ...
  const hitInterval =
    interval > 0 && stepIndex > 0 && stepIndex % interval === 0;
...
export async function executeLoop(
  ...
  const autoApprove =
    context.evalMode === true && process.env.EVAL_AUTO_APPROVE === "true";
  ...
  while (stepIndex < MAX_EXECUTION_STEPS) {
    ...
    if (Date.now() - loopStart > EXECUTOR_TIMEOUT_MS) {
      runLog.warn(
        { elapsed_ms: Date.now() - loopStart, stepIndex },
        "Executor: wall-clock timeout — aborting",
      );
      ...
      return {
        finalResponse:
          "This request took too long to complete and was stopped. Please try a simpler or more specific request.",
        tokenUsage,
      };
    }
    ...
    const budgeted = fitMessagesToBudget(
      ...
      {
        model: context.model,
        reserveOutputTokens: 1024 + observationTokens + toolDefsTokens,
      },
    );
    ...
    recordLLMRequestPayload(runLog, "executor", llmMessages, {
      runId,
      stepIndex,
      model: context.model,
    });
    ...
    const contentPreview = response.content
      ? response.content.slice(0, 500) +
        (response.content.length > 500 ? "... [truncated]" : "")
      : null;
    runLog.debug(
      {
        stepIndex,
    ...
    if (isToolCall) {
      ...
      for (const tc of response.tool_calls!) {
        toolCallSignatures.push(
          toolCallSignature({ name: tc.name, arguments: tc.arguments }),
        );
        ...
        if (decision.abort) {
          ...
          return {
            finalResponse:
              "I encountered a processing loop while trying to complete your request. Please try rephrasing your question.",
            tokenUsage,
          };
        }
      }
      ...
      const announce = (toolCall: LLMToolCall) =>
        publishRunEvent(runId, {
          type: "tool.call",
          data: { tool_name: toolCall.name, args: toolCall.arguments },
        });
      ...
      const recordResult = async (
        ...
        const trust =
          toolResult.trust ??
          toolRegistry.get(toolCall.name)?.trust ??
          "untrusted";
...
${JSON.stringify(toolResult.output)}
      ...
      if (runConcurrently) {
        const plainFlags = await Promise.all(
          toolCalls.map((tc) => isPlainServerTool(tc.name)),
        );
      ...
      if (runConcurrently) {
        runLog.debug(
          ...
          "Executor: running independent tool batch in parallel",
        );
      ...
      if (shouldReplan(context, batchResults, stepIndex)) {
        ...
        runLog.info(
          { stepIndex, hadError: batchResults.some((r) => !r.success) },
          "Executor: triggering re-planner",
        );
        ...
        try {
          ...
          if (replanResult.revised && replanResult.plan) {
            ...
            await prisma.run.update({
              where: { id: runId },
              data: {
                plan: replanResult.plan as unknown as Prisma.InputJsonValue,
              },
            });
            await publishRunEvent(runId, {
              type: "plan.updated",
            ...
            runLog.info(
              {
                newSteps: replanResult.plan.steps.length,
                reason: replanResult.reason,
              },
            ...
            runLog.info(
              { reason: replanResult.reason },
              "Executor: re-planner confirmed plan is still valid",
            );
          }
        } catch (replanErr) {
          ...
          runLog.warn(
            { err: replanErr },
            "Executor: re-planner failed — continuing with original plan",
          );
        }
      }
      ...
      const extractedToolCall = tryExtractToolCallFromText(
        response.content,
        tools,
      );
      ...
      if (extractedToolCall) {
        ...
        runLog.debug(
          {
            ...
            toolArgs: extractedToolCall.arguments,
          },
        ...
        const extractedTrust =
          toolResult.trust ??
          toolRegistry.get(extractedToolCall.name)?.trust ??
          "untrusted";
        let toolOutput: string;
        if (!toolResult.success) {
          toolOutput = `TOOL ERROR: ${toolResult.error}. Report this failure to the user. Do NOT make up data.`;
...
${JSON.stringify(toolResult.output)}
        ...
        return {
          finalResponse: stripLatexFormatting(response.content),
          tokenUsage,
        };
      }
    } else {
      ...
      if (hasToolResults) {
        runLog.debug(
          { stepIndex },
          "Executor: null content after tool results — attempting forced summarization",
        );
        const nudge: LLMMessage = {
          role: "user",
          content:
            "Based on the tool results above, provide a complete and clear response to the user. If additional tool calls are still needed to fulfill the request, make them now.",
        };
        ...
        try {
          ...
          const summaryResponse = await llmClient.chat(summarizationMessages, {
            think: false,
          });
          ...
          if (summaryResponse.content) {
            ...
            return {
              finalResponse: stripLatexFormatting(summaryResponse.content),
              tokenUsage,
            };
          }
          runLog.debug(
            { stepIndex },
            "Executor: forced summarization also returned null — falling back to heuristic",
          );
        } catch (err) {
          runLog.warn(
            { err },
            "Executor: forced summarization failed — falling back to heuristic",
          );
        }
      }
      // Fall back to key-result extraction or generic message
      const lastToolObs = observations.filter((o) => o.role === "tool").pop();
      const keyResult = lastToolObs
        ? extractKeyResult(lastToolObs.content)
        : null;
      const finalResponse = keyResult
        ? `The result is ${keyResult}.`
        : "Task completed.";
      logCanonical(runLog, "Executor: completed", {
        duration_ms: Date.now() - loopStart,
        status: "success",
  ...
  return {
    finalResponse:
      "Maximum execution steps reached. Please try again with a simpler request.",
    tokenUsage,
  };
}
