...
export async function runSubagent(
  ...
  if (parentDepth >= MAX_DEPTH) {
    return {
      success: false,
      error: `Sub-agent depth limit (${MAX_DEPTH}) reached; cannot spawn further sub-agents.`,
    };
  }
  if (active >= MAX_CONCURRENT) {
    return {
      success: false,
      error: `Too many concurrent sub-agents (max ${MAX_CONCURRENT}); run them sequentially.`,
    };
  }
  ...
  try {
    ...
    return await new Promise<SubagentResult>((resolve) => {
      ...
      if (!redisUrl) {
        resolve({
          success: false,
          error: "Redis not available for sub-agent orchestration",
          ...base,
        });
        return;
      }
      ...
      const timer = setTimeout(() => {
        ...
        resolve({
          success: false,
          status: "timeout",
          error: `Sub-agent timed out after ${TIMEOUT_MS}ms`,
          ...base,
        });
      }, TIMEOUT_MS);
      ...
      subscriber.subscribe(channel, (err) => {
        if (err) {
          ...
          resolve({
            success: false,
            error: `Failed to subscribe to sub-agent channel: ${err.message}`,
            ...base,
          });
        }
      });
      ...
      enqueueRunExecution({
        ...
        .then((jobId) => {
          if (!jobId) {
            ...
            resolve({
              success: false,
              error: "Failed to enqueue sub-agent run (queue unavailable)",
              ...base,
            });
          }
        })
        .catch((e) => {
          ...
          resolve({
            success: false,
            error: `Failed to enqueue sub-agent: ${e instanceof Error ? e.message : "unknown"}`,
            ...base,
          });
        });
    });
