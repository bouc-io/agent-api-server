...
export class OllamaClient implements LLMClient {
    ...
    async chat(
        ...
        for (let attempt = 1; attempt <= LLM_MAX_RETRIES; attempt++) {
            try {
                ...
                log.warn({ attempt, maxRetries: LLM_MAX_RETRIES, delay: delayMs }, 'LLM request failed, retrying');
    ...
    private async doChat(
        ...
        const contentPreview = llmResponse.content
            ? llmResponse.content.slice(0, 500) + (llmResponse.content.length > 500 ? '... [truncated]' : '')
            : null;
        log.debug(
            {
                contentPreview,
                toolCalls: llmResponse.tool_calls?.map((tc) => ({ name: tc.name, arguments: tc.arguments })),
