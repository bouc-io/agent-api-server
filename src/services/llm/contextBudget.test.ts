...
describe('fitMessagesToBudget', () => {
  ...
  const mk = (n: number, len = 40): LLMMessage[] =>
    Array.from({ length: n }, (_, i) => ({ role: i % 2 ? 'assistant' : 'user', content: 'x'.repeat(len) }));

  it('returns messages unchanged when within budget', () => {
    const msgs = [sys, ...mk(3)];
    const r = fitMessagesToBudget(msgs, { contextWindow: 100_000, reserveOutputTokens: 1024 });
  ...
  it('drops oldest non-system messages when over budget, keeping system + recent', () => {
    ...
    const r = fitMessagesToBudget(msgs, { contextWindow: 600, reserveOutputTokens: 100 });
  ...
  it('never drops below system + the last message even if still over budget', () => {
    ...
    const r = fitMessagesToBudget([sys, huge], { contextWindow: 100, reserveOutputTokens: 10 });
  ...
  it('counts tool_call payloads in the estimate', () => {
    const withTools: LLMMessage[] = [
      { role: 'assistant', content: '', tool_calls: [{ id: '1', name: 'calc', arguments: { expression: '2+2' } }] },
    ];
