...
export class OpenAIClient implements LLMClient {
    ...
    async chat(messages: LLMMessage[], options?: LLMChatOptions): Promise<LLMResponse> {
        ...
        log.debug({ model: this.config.model, provider: this.config.provider, messageCount: messages.length }, 'OpenAIClient: sending chat');
        ...
        const toolCalls: LLMToolCall[] | undefined = message?.tool_calls?.map((tc: any) => ({
        ...
        return {
            ...
            finish_reason: (finishReason === 'tool_calls' ? 'tool_calls' : 'stop') as LLMResponse['finish_reason'],
            usage: {
