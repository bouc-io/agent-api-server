...
const cfg = (provider: LLMProviderConfig['provider']): LLMProviderConfig => ({ ...base, provider });
...
describe('createLLMClient routing', () => {
  ...
  it.each(['openai', 'google', 'azure'] as const)('routes %s -> OpenAIClient', (p) => {
  ...
  it('falls back to BoucioClient for an unknown provider', () => {
    expect(createLLMClient(cfg('something-else' as LLMProviderConfig['provider']))).toBeInstanceOf(
      BoucioClient
    );
  });
});
