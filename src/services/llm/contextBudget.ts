 ...
 */
export const DEFAULT_CONTEXT_WINDOW = parseInt(process.env.OLLAMA_NUM_CTX || '4096', 10);
 ...
 */
export function fitMessagesToBudget(messages: LLMMessage[], options: BudgetOptions = {}): BudgetResult {
  ...
  let kept = rest.slice();
  let dropped = 0;
  while (kept.length > 1 && estimateMessagesTokens([...system, ...kept]) > budget) {
    kept.shift();
    dropped++;
  }
 ...
 */
export function estimateToolDefsTokens(tools: LLMToolDefinition[] | undefined | null): number {
