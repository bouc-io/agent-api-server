...
const OLLAMA_REPEAT_PENALTY = parseFloat(process.env.OLLAMA_REPEAT_PENALTY || '1.15');
...
export class BoucioClient implements LLMClient {
    ...
    constructor(config?: LLMProviderConfig | null) {
        ...
        this.axiosInstance = axios.create({
            httpsAgent: new https.Agent({ rejectUnauthorized: !ALLOW_SELF_SIGNED_CERTS }),
        });
    }
    ...
    private async chatWithRetries(messages: LLMMessage[], options?: LLMChatOptions): Promise<LLMResponse> {
    ...
    private async doChat(messages: LLMMessage[], options?: LLMChatOptions): Promise<LLMResponse> {
        ...
        log.debug({ model: this.model, messageCount: messages.length }, 'BoucioClient: sending chat');
        ...
        const toolCalls: LLMToolCall[] | undefined = data.message?.tool_calls?.map((tc) => ({
