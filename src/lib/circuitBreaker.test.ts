...
describe('CircuitBreaker', () => {
  ...
  it('opens after the failure threshold and then fails fast', async () => {
    const cb = new CircuitBreaker('t', { failureThreshold: 3, cooldownMs: 1000, now: () => 0 });
    for (let i = 0; i < 3; i++) {
    ...
    let called = false;
    await expect(cb.execute(async () => { called = true; return 'x'; })).rejects.toBeInstanceOf(CircuitOpenError);
    expect(called).toBe(false);
  });

  it('transitions to half-open after cooldown and closes on a successful trial', async () => {
    let t = 0;
    const cb = new CircuitBreaker('t', { failureThreshold: 1, cooldownMs: 100, now: () => t });
