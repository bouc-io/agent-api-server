...
export const createRun = async (req: Request, res: Response) => {
    ...
    const { agent_id = 'default', trigger_message_id, client_type, eval_mode } = req.body;
            ...
            if (!triggerMessage) {
                return res.status(400).json({
                    error: {
                        ...
                        message: 'Trigger message not found or does not belong to this assignment',
                    },
                });
            }
        }
...
export const submitToolResult = async (req: Request, res: Response) => {
    ...
    try {
        ...
        res.status(500).json({
            error: { code: 'INTERNAL_ERROR', message: 'Failed to submit tool result' },
        });
    }
};
...
export const approveRun = async (req: Request, res: Response) => {
    ...
    const assignmentId = String(req.params.assignmentId);
    const runId = String(req.params.runId);
    const { approval_request_id, approved, reason } = req.body as ApproveRunRequest;

    if (!approval_request_id || typeof approved !== 'boolean') {
        return res.status(400).json({
            error: { code: 'BAD_REQUEST', message: 'approval_request_id and approved (boolean) are required' },
        });
    }
    ...
    try {
        ...
        await redis.publish(channel, JSON.stringify({ approved, reason: reason ?? null }));

        log.info({ runId, approvalRequestId: approval_request_id, approved, decidedBy: userId }, 'Approval decision submitted');

        res.status(204).send();
    } catch (error) {
        log.error({ err: error }, 'Error submitting approval decision');
        res.status(500).json({
            error: { code: 'INTERNAL_ERROR', message: 'Failed to submit approval decision' },
        });
    }
};
...
export const listApprovals = async (req: Request, res: Response) => {
    ...
    try {
        const run = await prisma.run.findFirst({
            where: { id: runId, assignment_id: assignmentId, assignment: { user_id: userId } },
            select: { id: true },
        });
...
export const submitFeedback = async (req: Request, res: Response) => {
    ...
    if (!rating || !['positive', 'negative'].includes(rating)) {
        return res.status(400).json({
            error: { code: 'BAD_REQUEST', message: "rating must be 'positive' or 'negative'" },
        });
    }
